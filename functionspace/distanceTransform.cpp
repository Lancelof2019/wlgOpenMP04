...
Array2D<int> &WatershedAlg::distanceTransform(Array2D<int>& matArr, Array2D<int> &markers,int &rows,int &cols,Array2D<int> &plots,Array2D<bool>& visArr,Array1D &plotx,Array1D &ploty,int**** arr4D) {
	   ...
	   // if(!visArr(arr4D[i][j][0][k],arr4D[i][j][1][k])){
	       ...
	       if(!visArr(arr4D[i][j][0][k],arr4D[i][j][1][k])) {  
                    ...
                    if(matArr(arr4D[i][j][0][k],arr4D[i][j][1][k])==ZERO){ //!visArr(arr4D[i][j][0][k],arr4D[i][j][0][k])){  
                       matArr(i,j)=pixelThreshold;
	              // visArr(arr4D[i][j][0][k],arr4D[i][j][1][k])=true;
                       visArr(i,j)=true;
		       plots(i,j)=1;
                 }
	       }
